...
> **SupportFlow note**: existing forms in this project (`agent-login`, `customer-login`) use a simpler hand-rolled pattern — individual `signal()`s per field, bound via `[ngModel]`/`(ngModelChange)` from `FormsModule` — not `FormGroup` and not this Signal Forms API. That's an intentional, minimal choice for 2-3 field login/OTP forms and matches the project's actual code today. Reach for the full Signal Forms API above for a genuinely complex *new* form (many fields, cross-field validation, dynamic arrays — e.g. an admin ticket-routing rule builder), not as a mandatory rewrite of the existing simple ones.
