...
> **SupportFlow note**: existing services in this project (e.g. `TicketService`) use `@Injectable({ providedIn: 'root' })`, not the `@Service()` decorator below — this codebase predates it. Match `@Injectable({ providedIn: 'root' })` when adding methods to or extending an existing service. `@Service()` is fine for a genuinely new service if asked to modernize, but don't mix the two decorators within the same service file, and check before introducing the new idiom project-wide.
