...
> **SupportFlow note**: `@angular/material`/`@angular/cdk` are not installed in this project, so the `MatButtonHarness`-style examples below don't apply as-is. Writing a harness for one of this project's own custom components would mean authoring a `ComponentHarness` subclass with `@angular/cdk/testing` directly (not covered by the examples here) — until that's worth the setup cost, prefer the direct `fixture`-based approach in `testing-fundamentals.md`.
