...
> **SupportFlow note**: this project uses neither strategy below as-is — `API_BASE_URL`, `AGENT_APP_URL`, and `CUSTOMER_APP_URL` are plain exported constants in `projects/shared/src/api.config.ts` / `app-urls.ts` (imported as `@supportflow/shared`), hardcoded to localhost ports for local dev. There's no `environments/` folder and no `ng generate environments` run in this repo. Don't introduce one for a new value — add it to the existing shared constants files instead, unless you're deliberately setting up per-environment builds (dev/staging/prod), in which case ask first since it's a repo-wide structural change.
